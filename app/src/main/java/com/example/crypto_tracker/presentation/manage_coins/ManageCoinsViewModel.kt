...
@HiltViewModel
class ManageCoinsViewModel @Inject constructor(
    private val repository: CryptoRepository
) : ViewModel() {

    private val _state = MutableStateFlow(ManageCoinsState())
    val state = _state.asStateFlow()

    init {
        viewModelScope.launch {
            repository.getUserCoins().collect { coins ->
                _state.update { it.copy(userCoins = coins) }
            }
        }
    }
